...
Definen y validan el payload de las dos acciones expuestas por el agente A2A:
...
class ProposeRequest(BaseModel):
    ...
    @classmethod
    def _at_least_two_emails(cls, v: list[str]) -> list[str]:
        if not isinstance(v, list) or len(v) < 2:
            raise ValueError("'participants' must be an array with at least 2 emails")
        return v
